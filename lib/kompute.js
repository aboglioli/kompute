  ...
  const wrapItem = (item, prefix = '') => {
    ...
    const handlers = {
      ...
      set: (target, key, value) => {
        // It doesn't update array data before returning
        target[key] = value;

        // Update data in array
        const path = `${prefix}.${key}`;
        // const [, element, prop] = getElement({ getId, arr }, path);
        // element[prop] = value;

        // Notify changes and update dependent items
        const dependentNodes = findDependentNodes({ tree }, path);
    ...
    item = Object.entries(item).reduce((newObj, [key, value]) => {
      if (typeof value === 'object') {
        if (!isKomputeObject(value)) {
          ...
          if (tree.some(node => node.dependsOn.includes(path))) {
